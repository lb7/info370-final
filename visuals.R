...
n <- mydata %>%
  mutate(
    ...
    rating.d = predict(director_Rating),
    revenue.d = predict(director_Revenue)
  )
...
# Plot director gender vs ratings
v.dir.ratings <- n %>% 
  filter(director_gender != 0) %>%
  mutate(director_gender = factor(director_gender, levels=c(1,2), labels=c('Female', 'Male'))) %>%
  ggplot() +
  geom_jitter(aes(director_gender, averageRating, color=director_gender)) +
  geom_line(aes(as.numeric(director_gender), rating.d), size=1.2) + 
  labs(title='Rating by Director Gender', x='Director Gender', y='Average Rating')

# Plot director gender vs revenue
v.dir.revenue <- n %>% 
  filter(director_gender != 0) %>%
  mutate(director_gender = factor(director_gender, levels=c(1,2), labels=c('Female', 'Male'))) %>%
  ggplot() +
  geom_jitter(aes(director_gender, revenue, color=director_gender)) +
  geom_line(aes(as.numeric(director_gender), revenue.d), size=1.2) + 
  labs(title='Revenue by Director Gender', x='Director Gender', y='Revenue ($)')
